/**
 * ISO 3166-1 alpha-2 countries for the self-serve assessment.
 *
 * Kept as a compact string and parsed once at module load so the select is
 * deterministic (no Intl.DisplayNames dependency) without costing much weight.
 * Codes are what we send to the Worker; scoring.ts matches on them directly.
 */
...
export const COUNTRIES: Country[] = RAW.split('|').map((entry) => {
  ...
  return { code: entry.slice(0, split), name: entry.slice(split + 1) };
});

/** The markets we sell into first, surfaced above the full list. */
